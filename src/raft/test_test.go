...
func TestFailAgree2B(t *testing.T) {
	...
	Debug(dError, "S%d disconnect", leader+1)
	...
	Debug(dError, "S%d re-connect", leader+1)
...
func TestBackup2B(t *testing.T) {
	...
	Debug(dTrace, "\n disconnect %d %d %d ", (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	...
	cfg.disconnect((leader1 + 1) % servers)
	Debug(dTrace, "\n disconnect %d %d", (leader1 + 0) % servers, (leader1 + 1) % servers)
	...
	Debug(dTrace, "\n connect %d %d %d ", (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	...
	cfg.connect(other)
	Debug(dTrace, "\n connect %d %d %d ", (leader1 + 0) % servers, (leader1 + 1) % servers, other)
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			Debug(DTest, "S%d disconnected", victim)
		...
		if disconnect {
			...
			cfg.connect(victim)
			Debug(DTest, "S%d reconnected", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
			...
			cfg.connect(victim)
			Debug(DTest, "S%d restarted", victim)
			cfg.one(rand.Int(), servers, true)
			leader1 = cfg.checkOneLeader()
		}
	}
